...
36. Apply fixed base scores:
   ...
   - CSS Selector: 75
   - XPath: 65
37. Prefer any unique locator over every non-unique locator.
38. If no candidate is unique, choose the highest-priority locator type and then the candidate with the lowest match count within that type.
39. Mark non-unique selections as `Non-Unique` while retaining their base score.
40. Apply deterministic tie-breaking for candidates with equal priority and match count.
41. Report only the selected locator, locator type, match count, and score.
...
- Confidence: high - implementation artifact availability can be verified deterministically.
